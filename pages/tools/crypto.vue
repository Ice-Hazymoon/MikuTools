<template>
    <div class="crypto">
        <nya-container title="在线加密解密">
            <nya-input v-model="content" rows="5" type="textarea" autofocus autocomplete="off" label="待处理的内容" placeholder="请输入要处理的内容" />
            <nya-input v-if="ciphersList[encryption] || HMACList.includes(encryption)" v-model="key" autocomplete="off" :label="HMACList.includes(encryption) ? 'salt' : '密钥'" :placeholder="`请输入${HMACList.includes(encryption) ? 'salt' : '密钥'}`" />
            <nya-input v-if="encryption === 'RC4Drop'" v-model="drop" type="number" autocomplete="off" label="drop" placeholder="RC4Drop特有参数，请输入要丢弃的密匙初始数量" />
            <br>
            <nya-select v-if="!type" v-model="encryption" :items="encryptionList" label="选择加密方式" />
            <nya-select v-else v-model="encryption" :items="ciphersList" label="选择解密方式" />
            <br>
