<template>
    <div class="relatives_name">
        <nya-container title="亲戚称谓&关系计算器">
            <nya-select v-model="type" :items="{'default': '算称谓', 'chain': '找关系'}" label="选择处理方式" />
            <nya-input v-show="type === 'default'" v-model="content" rows="5" type="textarea" autofocus autocomplete="off" label="要找的称谓" placeholder="如：爸爸的奶奶" />
            <nya-input v-show="!(type === 'default')" v-model="chain" rows="5" type="textarea" autofocus autocomplete="off" label="要找的关系" placeholder="如：奶奶" />
            ...
            <div v-show="type === 'default'">
                <nya-radio v-model="sex" name="sex" :value="0" label="我是女的" />
                <nya-radio v-model="sex" name="sex" :value="1" label="我是男的" />
            </div>
        </nya-container>
